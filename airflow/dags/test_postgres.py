...
def cargar_default():
    ...
    hook = PostgresHook(postgres_conn_id="postgres_local")
    df = hook.get_pandas_df(f"SELECT * FROM {tabla};")
    df.to_csv("/tmp/archivo_trigger.csv", index = False)
    logging.info("Datos finales cargados correctamente.")
...
def tarea_final():
    logging.info("Esta tarea se ejecuta aunque existan fallos.")
...
with DAG(
    ...
    email_alerta = EmailOperator(
        ...
        to = "yanttzeenlayvictorio@gmail.com",
    ...
    esperar_archivo = FileSensor(
        ...
        filepath = "/tmp/archivo_trigger.csv",
        poke_interval = 10,
        timeout = 60,
        mode = "poke"
    )
    cargar_default = PythonOperator(
        task_id ="cargar_default",
        python_callable = cargar_default
    )
 # comentario
inicio >> test >> crear >> cargar_default >> esperar_archivo >> insertar >> leer >> leer_postgres >> transformar_datos >> crear_tabla_destino >> cargar_datos_transformados >> falla >> email_alerta >> tarea_final >> fin
